...
def test_net_on_dataset(multi_gpu=False):
    ...
    if multi_gpu:
        ...
        all_boxes, all_segms, all_keyps, all_refined_segms, all_refined_keyps = \
...
def multi_gpu_test_net_on_dataset(num_images, output_dir):
    ...
    all_refined_segms = [[] for _ in range(cfg.MODEL.NUM_CLASSES)]
    all_refined_keyps = [[] for _ in range(cfg.MODEL.NUM_CLASSES)]
    for det_data in outputs:
        all_boxes_batch = det_data['all_boxes']
        all_segms_batch = det_data['all_segms']
        all_keyps_batch = det_data['all_keyps']
        all_refined_segms_batch = det_data['all_refined_segms']
        all_refined_keyps_batch = det_data['all_refined_keyps']
        for cls_idx in range(1, cfg.MODEL.NUM_CLASSES):
            ...
            all_refined_segms[cls_idx] += all_refined_segms_batch[cls_idx]
            all_refined_keyps[cls_idx] += all_refined_keyps_batch[cls_idx]
...
def empty_results(num_classes, num_images):
    ...
    all_refined_segms = [[[] for _ in range(num_images)] for _ in range(num_classes)]
    all_refined_keyps = [[[] for _ in range(num_images)] for _ in range(num_classes)]
    return all_boxes, all_segms, all_keyps, all_refined_segms, all_refined_keyps
