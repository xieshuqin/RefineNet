...
def add_refine_keypoints_blobs(
    ...
    if cfg.MODEL.USE_GAUSSIAN_HEATMAP:
        add_refine_keypoints_blobs_gaussian(
            blobs, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx, data
        )
    elif cfg.MODEL.USE_SIGMOID_HEATMAP:
        add_refine_keypoints_blobs_sigmoid(
            blobs, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx, data
        )
    else:
        add_refine_keypoints_blobs_softmax(
            blobs, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx, data
        )


def add_refine_keypoints_blobs_gaussian(
        blobs, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx, data
    ):
...
def add_refine_keypoints_blobs_sigmoid(
        blobs, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx, data
    ):
    """Add Mask R-CNN keypoint specific blobs to the given blobs dictionary."""
    # Note: gt_inds must match how they're computed in
    ...
    if kp_fg_inds.shape[0] > 0:
        ...
        shape = pad_fg_rois.shape[0] * cfg.KRCNN.NUM_KEYPOINTS
