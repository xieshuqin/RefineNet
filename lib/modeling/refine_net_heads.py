...
def add_refine_net_keypoint_losses(model, blob_refined_keypoint):
    if cfg.MODEL.USE_GAUSSIAN_HEATMAP:
        # Use gaussian heatmap, MSE loss
        return add_refine_net_keypoint_losses_gaussian(model, blob_refined_keypoint)
    elif cfg.MODEL.USE_SIGMOID_HEATMAP:
        # Sigmoid loss
        return add_refine_net_keypoint_losses_sigmoid(model, blob_refined_keypoint)
    else:
        # default use softmax loss
        return add_refine_net_keypoint_losses_softmax(model, blob_refined_keypoint)


def add_refine_net_keypoint_losses_sigmoid(model, blob_refined_keypoint):
    """Add Mask R-CNN keypoint specific losses. Using Sigmoid loss"""
    loss_refined_kps = model.net.SigmoidCrossEntropyLoss(
        [blob_refined_keypoint, 'refined_keypoint_locations_int32'], 
        'loss_refined_kps',
