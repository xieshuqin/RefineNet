...
REGISTER_CPU_OPERATOR(
    ...
    ThresholdSigmoidCrossEntropyLossOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(
    ThresholdSigmoidCrossEntropyLossGradient,
    ThresholdSigmoidCrossEntropyLossGradientOp<float, CPUContext>);

OPERATOR_SCHEMA(ThresholdSigmoidCrossEntropyLoss)
    .NumInputs(2)
    .NumOutputs(1)
    .SetDoc(R"DOC(
...
and gradient computation.
    ...
    .Arg(
        "threshold",
        "(float) default 0.5; only probability >= threshold will account "
        "for loss.")
    .Input(
        0,
        "X",
...
OPERATOR_SCHEMA(ThresholdSigmoidCrossEntropyLossGradient)
    .NumInputs(3)
    .NumOutputs(1)
    .Input(
        0,
...
class GetThresholdSigmoidCrossEntropyLossGradient : public GradientMakerBase {
...
REGISTER_GRADIENT(
    ThresholdSigmoidCrossEntropyLoss, 
    GetThresholdSigmoidCrossEntropyLossGradient
);
