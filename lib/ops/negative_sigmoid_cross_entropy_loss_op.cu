...
#include "negative_sigmoid_cross_entropy_loss_op.h"
...
__global__ void NegativeSigmoidCrossEntropyLossKernel(
    const int n,
    const float* logits,
    const int* targets,
    float* losses,
...
__global__ void NegativeSigmoidCrossEntropyLossGradientKernel(
    const int n,
    const float* logits,
    const int* targets,
    float* d_logits,
...
template <>
bool NegativeSigmoidCrossEntropyLossOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);
  auto& T = Input(1);
  ...
  NegativeSigmoidCrossEntropyLossKernel<<<
...
template <>
bool NegativeSigmoidCrossEntropyLossGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);
  auto& T = Input(1);
  ...
  counts_.ResizeLike(X);
  normalizer_.Resize(vector<TIndex>());
  NegativeSigmoidCrossEntropyLossGradientKernel<<<
      CAFFE_GET_BLOCKS(X.size()),
      CAFFE_CUDA_NUM_THREADS,
      0,
      context_.cuda_stream()>>>(
...
REGISTER_CUDA_OPERATOR(
    NegativeSigmoidCrossEntropyLoss,
    NegativeSigmoidCrossEntropyLossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(
    NegativeSigmoidCrossEntropyLossGradient,
    NegativeSigmoidCrossEntropyLossGradientOp<float, CUDAContext>);
} // namespace caffe2
