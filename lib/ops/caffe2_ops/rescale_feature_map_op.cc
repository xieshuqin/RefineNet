...
template <>
bool RescaleFeatureMapOp<float, CPUContext>::RunOnDevice() {
...
REGISTER_CPU_OPERATOR(RescaleFeatureMap, RescaleFeatureMapOp<float, CPUContext>);

#ifdef CAFFE2_HAS_MKL_DNN
REGISTER_MKL_OPERATOR(
    RescaleFeatureMap,
    mkl::MKLFallbackOp<RescaleFeatureMapOp<float, CPUContext>>);
#endif // CAFFE2_HAS_MKL_DNN

// Input: X, rois, data; Output: Y
OPERATOR_SCHEMA(RescaleFeatureMap)
    .NumInputs(3)
    .NumOutputs(1)
    .SetDoc(R"DOC(
Rescale Feature Map operation as used in RefineNet.
)DOC")
    .Arg(
        "spatial_scale",
    ...
    .Arg(
        "rescale_factor",
        "(float) default 1.0; Rescale factor of the input feature map X "
        "relative to the input image. E.g., 0.0625 if X has a stride of 16 "
        "w.r.t. the input image.")
    .Arg(
        "sampling_ratio",
    ...
    .Input(2, "Data", "4D input data of shape (N, C, H, W). ")
    .Output(
        0,
        "Y",
