...
#include "instance_weighted_sigmoid_cross_entropy_loss_op.h"
...
__global__ void InstanceWeightedSigmoidCrossEntropyLossKernel(
    const int n,
    const int per_size,
    const float* logits,
    const int* targets,
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    if (targets[index] == -1) {
      ...
      int batch_id = index / per_size;
      losses[index] =
          -1. * logits[index] * (targets[index] - (logits[index] >= 0)) +
          logf(
              1 +
              expf(logits[index] - 2 * logits[index] * (logits[index] >= 0)));
      losses[index] *= weights[batch_id];
      counts[index] = 1.;
    }
  }
}

__global__ void InstanceWeightedSigmoidCrossEntropyLossGradientKernel(
    const int n,
    const int per_size, 
    const float* logits,
    const int* targets,
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    if (targets[index] == -1) {
      ...
      d_logits[index] *= weights[batch_id];
      counts[index] = 1.;
    }
  }
}
...
template <>
bool InstanceWeightedSigmoidCrossEntropyLossOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);
  auto& T = Input(1);
  ...
  int per_size = X.size() / X.dim32(0);
  InstanceWeightedSigmoidCrossEntropyLossKernel<<<
...
template <>
bool InstanceWeightedSigmoidCrossEntropyLossGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);
  auto& T = Input(1);
  auto& Weights = Input(2);
  ...
  int per_size = X.size() / X.dim32(0);
  InstanceWeightedSigmoidCrossEntropyLossGradientKernel<<<
      CAFFE_GET_BLOCKS(X.size()),
      CAFFE_CUDA_NUM_THREADS,
      0,
      context_.cuda_stream()>>>(
...
REGISTER_CUDA_OPERATOR(
    InstanceWeightedSigmoidCrossEntropyLoss,
    InstanceWeightedSigmoidCrossEntropyLossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(
    InstanceWeightedSigmoidCrossEntropyLossGradient,
    InstanceWeightedSigmoidCrossEntropyLossGradientOp<float, CUDAContext>);
} // namespace caffe2
