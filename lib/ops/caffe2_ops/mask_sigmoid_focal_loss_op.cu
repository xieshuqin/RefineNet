...
__global__ void MaskSigmoidFocalLossKernel(
    const int n,
    const float gamma,
    const float* logits,
    const int* targets,
...
__global__ void MaskSigmoidFocalLossGradientKernel(
    const int n,
    const float gamma,
    const float* logits,
    const int* targets,
...
template<>
bool MaskSigmoidFocalLossOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0);
  auto& T = Input(1);
...
REGISTER_CUDA_OPERATOR(MaskSigmoidFocalLoss,
                       MaskSigmoidFocalLossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(MaskSigmoidFocalLossGradient,
                       MaskSigmoidFocalLossGradientOp<float, CUDAContext>);
} // namespace caffe2
