...
class GenerateMaskIndicatorsOp(object):
    ...
    def forward(self, inputs, outputs):
        ...
        height, width = floor(data.shape[2]*scale), floor(data.shape[3]*scale)
        ...
        mask_rois = (mask_rois*scale).astype(np.int32) # convert rois to int32

        for i in range(num_rois):
            mask_local = mask_probs[i]
            roi = mask_rois[i]
            shape = (roi[3]-roi[1]+1, roi[2]-roi[0]+1) # (roi_h, roi_w)
            mask_local_resize = cv2.resize(mask_local, shape)
            mask_indicators[i,roi[1]:roi[3],roi[0]:roi[2],:] = mask_local_resize
