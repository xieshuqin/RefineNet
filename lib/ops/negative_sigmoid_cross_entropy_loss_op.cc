...
REGISTER_CPU_OPERATOR(
    ...
    NegativeSigmoidCrossEntropyLossOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(
    NegativeSigmoidCrossEntropyLossGradient,
    NegativeSigmoidCrossEntropyLossGradientOp<float, CPUContext>);

OPERATOR_SCHEMA(NegativeSigmoidCrossEntropyLoss)
    .NumInputs(2)
    .NumOutputs(1)
    .SetDoc(R"DOC(
Negative Sigmoid Cross Entropy Loss. Only calculate loss for the negative targets.
...
OPERATOR_SCHEMA(NegativeSigmoidCrossEntropyLossGradient)
    .NumInputs(3)
    .NumOutputs(1)
    .Input(
        0,
...
class GetNegativeSigmoidCrossEntropyLossGradient : public GradientMakerBase {
...
REGISTER_GRADIENT(
    NegativeSigmoidCrossEntropyLoss, 
    GetNegativeSigmoidCrossEntropyLossGradient
);
